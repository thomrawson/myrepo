...
error_x_AZ <- function(x) {
  ...
  error_amt <- (sum((abs(y50_estimate_AZ_probability - y50_AZ_probability))^2  )  )#+  sum(abs(y100_estimate_AZ - y100_AZ)) + +  sum(abs(y25_estimate_AZ - y25_AZ)) )
  return(error_amt)
}


optim_AZ <- optimize(error_x_AZ, interval = c(0,2))
...
error_x_PF <- function(x) {
  ...
  error_amt <- (sum( (abs(y50_estimate_PF - y50_PF))^1  ) ) # +  sum(abs(y100_estimate_PF - y100_PF))  +  sum(abs(y25_estimate_PF - y25_PF)) )
  return(error_amt)
}
...
optim_infect <- optimize(error_x_infect, interval = c(0,4))
...
#Make a title
title <- ggdraw() + draw_label("AZ (1 dose) Vaccine Efficacy (x ~ 0.68)", fontface='bold')
#Add title:
