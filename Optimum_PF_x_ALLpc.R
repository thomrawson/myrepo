...
ggplot(data=Mapped_Data, aes(x=percent_mark, y=sev_disease_efficacy, group=1)) +
  ...
  geom_path(data = data.frame(percent_mark = c(100, 100), sev_disease_efficacy = c(0.6, sev_disease_efficacy[101]) ),
            size = 1, colour = "#EC7014",
            arrow = arrow(type = "open", angle = 40, length = unit(0.1, "inches")) 
  )+ylim(0.2,1) +
  theme_classic() +
...
ggplot(data=Mapped_Data, aes(x=percent_mark, y=sev_dis_efficacy, group=1)) +
  ...
  geom_path(data = data.frame(percent_mark = c(100, 100), sev_dis_efficacy = c(0.85, sev_dis_efficacy[101]) ),
            size = 1, colour = "#EC7014",
            arrow = arrow(type = "open", angle = 40, length = unit(0.1, "inches")) 
  )+ylim(0.2,1) +
  theme_classic() +
...
ggplot(data=Mapped_Data, aes(x=percent_mark, y=infection_efficacy, group=1)) +
  ...
  geom_path(data = data.frame(percent_mark = c(100, 100), infection_efficacy = c(0.45, infection_efficacy[101]) ),
            size = 1, colour = "#EC7014",
            arrow = arrow(type = "open", angle = 40, length = unit(0.1, "inches")) 
  )+ylim(0.2,1) +
  theme_classic() +
...
ggplot(data=Mapped_Data, aes(x=percent_mark, y=infection_efficacy, group=1)) +
  ...
  geom_path(data = data.frame(percent_mark = c(25, 25), infection_efficacy = c(0.85, infection_efficacy[26]) ),
            size = 1, colour = "#EC7014",
            arrow = arrow(type = "open", angle = 40, length = unit(0.1, "inches")) 
  )+
  geom_path(data = data.frame(percent_mark = c(100, 100), infection_efficacy = c(0.55, infection_efficacy[101]) ),
            size = 1, colour = "#EC7014",
            arrow = arrow(type = "open", angle = 40, length = unit(0.1, "inches")) 
  )+ ylim(0.2,1) +
  theme_classic() +
...
#Make a title
title <- ggdraw() + draw_label("PF Vaccine Efficacy (x ~ 0.762)", fontface='bold')
#Add title:
plot_grid(title, p, ncol=1, rel_heights=c(0.1, 1)) -> plot2 # rel_heights values control title margins
