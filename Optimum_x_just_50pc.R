...
error_x_AZ <- function(x) {
  ...
  error_amt <- (sum((abs(y50_estimate_AZ - y50_AZ))^2  )  )#+  sum(abs(y100_estimate_AZ - y100_AZ)) + +  sum(abs(y25_estimate_AZ - y25_AZ)) )
  return(error_amt)
}
...
#Make a title
title <- ggdraw() + draw_label("AZ Vaccine Efficacy (x ~ 0.39)", fontface='bold')
#Add title:
