...
error_x_infect <- function(x) {
  ...
  error_amt <- (sum(   (abs(y50_estimate_infect_probability - y50_infect_probability))^2 )    )  #+  sum(abs(y100_estimate_infect - y100_infect)) +  sum(abs(y25_estimate_infect - y25_infect)) )
  return(error_amt)
}
...
Table_Data <- data.frame(percent_mark = c(0, 25, 50, 100), infection_efficacy = c(0.45, 0.45, 0.2, 0))
...
ggplot(data=Mapped_Data, aes(x=percent_mark, y=infection_efficacy, group=1)) +
  ...
  labs(title="Efficacy against Infectiousness (AZ/PF/Mod (1 dose)) x~0.55",
       x ="Vaccine Escape Percentage", y = "VE") +
  geom_path(data = data.frame(percent_mark = c(50, 50), infection_efficacy = c(0.2, infection_efficacy[51]) ),
